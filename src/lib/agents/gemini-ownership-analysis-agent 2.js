...
// Feature flag for enhanced verification matching
const GEMINI_VERIFICATION_ENHANCED_MATCH = process.env.GEMINI_VERIFICATION_ENHANCED_MATCH === 'true';
...
/**
 * Build enhanced verification prompt with detailed explanation requirements
 */
function buildEnhancedVerificationPrompt(brand, existingResult, webSearchResultsText, searchQueries, webSnippets, debugMetadata) {
  return `
You are an expert corporate ownership analyst with deep expertise in corporate structures, ownership verification, and evidence quality assessment. Analyze the following web search results to determine the ownership of ${brand}.
...
EVIDENCE MATCHING INSTRUCTIONS:
For each verification requirement, you must:
- Identify specific snippets that support or contradict the claim
- Explain WHY the evidence supports or contradicts each requirement
- Rate the quality of evidence (strong, moderate, weak, insufficient)
- Note any gaps in evidence that prevent verification
...
\`\`\`json
{
  ...
  "explanations_by_requirement": {
    ...
    "family_control": {
      "status": "confirmed|contradicted|mixed_evidence|insufficient_evidence",
      "explanation": "Detailed explanation of family control verification",
      "evidence_quality": "strong|moderate|weak|insufficient",
      "supporting_snippets": ["specific snippet references"],
      "contradicting_snippets": ["specific snippet references"],
      "missing_information": ["what specific information is missing"]
    ...
    "percentage_ownership": {
      "status": "confirmed|contradicted|mixed_evidence|insufficient_evidence",
      "explanation": "Detailed explanation of percentage ownership verification",
      "evidence_quality": "strong|moderate|weak|insufficient",
      "supporting_snippets": ["specific snippet references"],
      "contradicting_snippets": ["specific snippet references"],
      "missing_information": ["what specific information is missing"]
    ...
    "corporate_structure": {
      "status": "confirmed|contradicted|mixed_evidence|insufficient_evidence",
      "explanation": "Detailed explanation of corporate structure verification",
      "evidence_quality": "strong|moderate|weak|insufficient",
      "supporting_snippets": ["specific snippet references"],
      "contradicting_snippets": ["specific snippet references"],
      "missing_information": ["what specific information is missing"]
    ...
    "geographic_control": {
      "status": "confirmed|contradicted|mixed_evidence|insufficient_evidence",
      "explanation": "Detailed explanation of geographic control verification",
      "evidence_quality": "strong|moderate|weak|insufficient",
      "supporting_snippets": ["specific snippet references"],
      "contradicting_snippets": ["specific snippet references"],
      "missing_information": ["what specific information is missing"]
...
/**
 * Build standard verification prompt (original behavior)
 */
function buildStandardVerificationPrompt(brand, existingResult, webSearchResultsText, searchQueries, webSnippets, debugMetadata) {
  return `
...
export async function performGeminiOwnershipAnalysis(brand, productName, existingResult) {
  try {
    ...
    console.log('[GEMINI_VERIFICATION] Raw Gemini input prompt length:', prompt.length);
    console.log('[GEMINI_VERIFICATION] Raw Gemini response:', text);
    console.log('[GEMINI_RAW_RESPONSE] Raw Gemini output:', {
      ...
      enhanced_match_enabled: GEMINI_VERIFICATION_ENHANCED_MATCH
    });
    ...
    let explanationsByRequirement = null;
    ...
    try {
      ...
      if (verificationResult?.verification_status) {
        ...
        // Extract enhanced explanation fields if available
        if (GEMINI_VERIFICATION_ENHANCED_MATCH && verificationResult.explanations_by_requirement) {
          explanationsByRequirement = verificationResult.explanations_by_requirement;
          console.log('[GEMINI_VERIFICATION] Enhanced explanations extracted:', {
            brand,
            requirements_analyzed: Object.keys(explanationsByRequirement),
            has_ultimate_ownership: !!explanationsByRequirement.ultimate_ownership,
            has_family_control: !!explanationsByRequirement.family_control,
            has_percentage_ownership: !!explanationsByRequirement.percentage_ownership,
            has_corporate_structure: !!explanationsByRequirement.corporate_structure,
            has_geographic_control: !!explanationsByRequirement.geographic_control
          });
        }
    ...
    const result = {
      ...
      ...(GEMINI_VERIFICATION_ENHANCED_MATCH && explanationsByRequirement && {
        gemini_analysis: {
          explanations_by_requirement: explanationsByRequirement,
          enhanced_match_enabled: true,
          verification_requirements_analyzed: Object.keys(explanationsByRequirement),
