...
export async function GET(request: NextRequest) {
  try {
    ...
    const dataSources = [
      {
        id: 'live',
        name: 'Live Scans',
        count: liveScans.length,
        accessible: true
      },
      {
        id: 'eval',
        name: 'Evaluation Tests',
        count: evalScans.length,
        accessible: evalScans.length > 0
      },
      {
        id: 'retry',
        name: 'Retry Runs',
        count: evalScans.filter(scan => scan.source_type === 'retry').length,
        accessible: evalScans.length > 0
      }
    ]
    ...
    console.error('[Evaluation V3 Metrics API] GET error:', error)
