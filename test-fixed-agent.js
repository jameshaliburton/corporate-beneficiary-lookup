...
async function testFixedAgent() {
  console.log('🧪 Testing Fixed Agent...\n')
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Agent test timed out after 15 seconds')), 15000)
    })
    ...
    if (error.message.includes('timed out')) {
      console.log('💡 The agent is still hanging somewhere')
    }
  }
}
