...
async function testOwnershipMappings() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      console.log(`   🏳️  Flag: ${(result as any).beneficiary_flag ?? 'N/A'}`);
      console.log(`   🔄 Result Type: ${result.result_type || 'N/A'}`);
      
      if ((result as any).ownership_flow?.length) {
        console.log(`   📊 Ownership Flow: ${(result as any).ownership_flow.join(' → ')}`);
      }
